...
[Download](#download) · [Installation](#installation) · [Documentation](docs/) · [Report Issue](https://github.com/Yutlanii/GeoAnnotate3D/issues)
...
### Deep Learning Dataset Export
- Direct export ready for PyTorch (RandLA-Net, PointNet++, KPConv)
- Automatic train/val/test split
- Optional export of a classified `.las` with standard **ASPRS** classification
  codes for interoperability with CloudCompare, QGIS, and ArcGIS
...
- **Resume training from any saved checkpoint** — safe to stop and continue
  a long training run
- Per-class IoU / precision / recall report at the end of training, not just
  an aggregate mIoU
...
- **Batch inference** — run a trained model over an entire folder of point
  clouds unattended
- Result visualization directly in the 3D canvas
- Correction with annotation tools and progressive retraining

### Usability
- In-app keyboard shortcuts reference (Help → Shortcuts)
- Window layout (docks/panels) is remembered between sessions
- Crash and error reports include a one-click "copy details" action for
  bug reports
...
Download the Windows installer from the [Releases section](https://github.com/Yutlanii/GeoAnnotate3D/releases/latest):
...
1. Download the `.exe` installer from [Releases](https://github.com/Yutlanii/GeoAnnotate3D/releases/latest)
2. Run the installer
...
@software{geoannotate3d_2026,
  ...
  year         = {2026},
  url          = {https://github.com/Yutlanii/GeoAnnotate3D},
  version      = {1.0.0}
}
```
...
[LinkedIn](https://www.linkedin.com/in/yutlani) · [GitHub](https://github.com/Yutlanii)
